...
## Подключение к Claude Code (плагин)

Репозиторий оформлен как плагин Claude Code: он ставится одной командой и приносит
сразу и MCP-сервер, и скилл `evateam` со сценариями работы. В отличие от подключения
сервера вручную, скилл и инструменты доступны из любой папки, а не только внутри
этого репозитория.

```bash
/plugin marketplace add schebotar/eva-mcp
/plugin install eva-mcp@eva-mcp
```

Плагин ставится копией репозитория, а зависимости и сборка в git не хранятся, поэтому
после установки (и после каждого обновления плагина) в его папке нужно один раз
выполнить:
...
Путь к установленной копии показывает `claude plugin details eva-mcp`. Разработчикам
удобнее запускать Claude Code с ключом `--plugin-dir <путь-до-клона>`: тогда плагин
берётся прямо из рабочей копии, где сборка и зависимости уже есть, а правки в
`skills/evateam/SKILL.md` подхватываются сразу.

Учётные данные сервер берёт из переменных окружения `EVA_URL` и `EVA_TOKEN`, а если их
нет — из файла `.env` рядом с самим пакетом. **Обрати внимание**: при установке
маркетплейса из локальной папки копируется всё её содержимое, включая `.env` с токеном —
копия окажется в каталоге плагинов. Если это нежелательно, задавай учётные данные
переменными окружения, а `.env` в клоне не держи.
...
├── .env.example
├── README.md
├── .claude-plugin/        # манифест плагина Claude Code и каталог маркетплейса
├── .mcp.json              # определение MCP-сервера для плагина
├── skills/
│   └── evateam/           # скилл со сценариями работы с EvaTeam
└── src/
