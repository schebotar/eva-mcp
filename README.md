...
- **`search_docs`** — поиск страниц wiki: по подстроке названия (`query`), корневые страницы wiki проекта (`project`) или подстраницы документа (`parent`). Дети ищутся по `parent.code` **и** `tree_parent.code` с дедупликацией — индекс дерева EvaWiki обновляется с задержкой, и у свежих страниц заполнено только одно из полей.
