...
Если сервер поставлен из npm, файла-образца рядом не будет: `files: ["dist"]` не тащит
в пакет ничего лишнего, включая `.env.example`. Учётные данные задаются переменными
окружения `EVA_URL` и `EVA_TOKEN` либо своим `.env` в корне установленного пакета —
сервер читает `.env` и из рабочей папки, и из корня пакета, так что запускать его можно
откуда угодно.
