...
#### Метрики и отчёты снова работают
`get_burndown_data`, `get_velocity`, `get_cycle_time`, `get_cumulative_flow`,
`get_sprint_review`, `get_sprint_retrospective`, `get_team_workload`,
`get_project_health` брали задачи запросом без фильтра — весь инстанс — и отбирали
нужное в памяти. На ~13 700 задачах это приводило к HTTP 502: четыре метрики просто
падали. Отчёты фильтровали задачи как `["parent", "==", <код проекта>]` — такой фильтр
молча возвращает 0 задач, поэтому все четыре отчёта всегда были пустыми.
