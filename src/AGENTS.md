...
списка в проекте (в `mcp-test` рядом живёт `BLOG-001875` «Лента»), поэтому нужен префикс
кода.
...
- ✅ `["lists.code", "IN", [sprintCode]]` — **работает**: используется в `search_tasks(sprint=…)`
  ...
  (`SPR-001617`) приходит полностью — 15 задач с архивом против 2 без него
- ❓ `["lists", "IN", [sprintCode]]` — не используется, поведение не проверялось

### Вывод
Фильтровать — **на сервере**: проект через `parent_id` с UUID (код резолвится в `getProject`),
спринт через `lists.code`. Выборка «весь инстанс, потом отобрать в памяти» упирается
в 502 — ровно так и было в метриках и отчётах, пока их не починили (issue #16).
