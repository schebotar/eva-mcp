...
### создание и движение по дереву wiki (CmfDocument)
- `parent` — контейнер страницы (раздел), у страниц wiki это **проект**: и у созданных
  в интерфейсе, и у созданных через API. Код страницы в `parent` — ошибка: документ создастся
  и прочитается, но в дереве навигации не появится (так было до 0.8.1, issue #43).
- `tree_parent` — позиция в дереве навигации, её видит пользователь в интерфейсе. Иерархию
  задаёт только это поле.
- ✅ `CmfDocument.create` принимает `parent` и `tree_parent` **одним вызовом** — проверено
  на живом инстансе: страница сразу создаётся с нужным `tree_parent`, страховочный `update`
  не нужен. Без `parent` документ уходит в личное пространство пользователя, поэтому
  `parent` = проект нужно указывать всегда.
- `tree_node_is_branch` — узел дерева является папкой (веткой), а не страницей.
- ⚠️ Страницы, созданные через MCP до 0.8.1, лежат с кодом страницы в `parent` и
  `tree_parent = null`: сами в дерево не встанут, нужен разовый перенос.
