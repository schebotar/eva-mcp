...
**Ветки.** Контрибьютор работает в `feature/*`, владелец — в `chore/*`. Прямые пуши в `master`
запрещены всем. База PR — `master`; стек допускается максимум один уровень и только на ветку
**открытого** PR. Если нижний PR закрыт, верхний переезжает на `master`.
