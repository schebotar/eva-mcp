...
**В тексте задачи — синтетика, а не стенд.** Разбирая реальный случай в issue, не приводим данные
рабочего инстанса: коды проектов и страниц, UUID, названия проектов и документов заказчика,
логины, значения счётчиков. `example-portal`, `DOC-001001` и «Раздел A» объясняют то же самое,
а issue читают контрибьюторы и их агенты. Исключение — таблица проверки в описании PR: там проект
нужен, иначе прогон не воспроизвести (см. «Проверка перед PR»), а сама проверка идёт по `mcp-test`.
