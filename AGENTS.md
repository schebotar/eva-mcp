...
**Правка тела issue или PR историю не стирает.** GitHub хранит предыдущие редакции и отдаёт их
через API (`userContentEdits`), поэтому «обезличить» утёкший текст редактированием нельзя: issue
удаляют и заводят заново, PR — закрывают и открывают новым. Решение об удалении — за владельцем:
удаление необратимо и чужие копии не отзывает.
