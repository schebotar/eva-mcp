...
- `EVA_URL` и `EVA_TOKEN` обязательны. Цепочка поиска — `src/helpers/credentials.ts`,
  ...
  3. `~/.eva-mcp` — умолчание пользователя (dotenv-формат, `os.homedir()`);
...
- Первый запуск без учётных данных создаёт `~/.eva-mcp` с шаблоном (права `0600` на POSIX),
  пишет в stderr, что заполнить, и завершается с кодом 1. Второй запуск работает.
