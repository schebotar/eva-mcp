...
- Первый запуск без учётных данных создаёт конфиг пользователя с шаблоном (каталог `0700`,
  файл `0600` на POSIX; `flag: "wx"` — существующий файл не трогается), пишет в stderr
