...
**Проверка перед PR.** Обязательно: `npx tsc --noEmit` и прогон **собранного** сервера по stdio
(`initialize` + `tools/list`) — это ловит модуль, не подключённый в `src/index.ts`, и рассинхрон
`inputSchema`. Живые пробы: читать можно на любом проекте, писать — только в `mcp-test`; временные
скрипты не коммитить. Проверять свежим спавном процесса, а не через MCP-сессию редактора: там до
перезапуска живёт старая сборка из `dist`. В описании PR — команда, проект и результат.
